...
一个面向 RenpyThief 的独立社区启动器：把翻译请求路由到用户自己的 AI API 或专业机器翻译平台。

> [!IMPORTANT]
> 本项目与 RenpyThief 官方没有隶属、授权或合作关系。补丁使用目录 `6.7.8Origin` 中附带的干净 RenpyThief 6.7.8，启动时复制到本机工作副本，不读写你电脑上已有的原版安装。使用本补丁仍须遵守原软件、游戏和翻译平台的服务条款。

> [!WARNING]
> 当前版本只实测了 **RenpyThief 6.7.8（x86 / Qt 5.15.2）**。其他版本可能无法启动、无法保护版本检查或无法激活翻译路由。翻译走用户自己的服务，不使用官方翻译额度。

## 功能概览

- 填写自己的 API 后启动；不会连官方翻译服务，也不会产生官方额度费用。
...
- 默认精确保护 RenpyThief 已知版本检查；启动后还会本地应答登录/心跳/注入上报。关闭前会明确警告。
- 不修改系统代理、DNS、hosts、证书、防火墙或全局网络设置。
- 默认 Bridge 日志不记录游戏原文、译文、Authorization、Cookie 或 API Key。
- **v1.1.0**：主进程解出台词后改送到本机 Bridge，再按官方回包格式写回游戏。游戏窗口和原版翻译仪表盘都会显示这次翻译。界面已去掉官方额度选项。
...
两种版本功能相同，均已包含 GUI、翻译 Bridge、原生组件，以及所用的干净 RenpyThief 6.7.8（`6.7.8Origin`）。不需要另外安装 Python。
...
- 补丁使用内置的干净 RenpyThief 6.7.8，不依赖你电脑上是否已经装过原版。
- 兼容性保护会在进程内应答官方会话接口；需要对应平台的有效凭据，本机 OpenAI 兼容服务可将 API Key 留空。
- 本地端口 `127.0.0.1:19899` 未被其他程序占用。
- 启动时会把内置 6.7.8 复制到 `%LOCALAPPDATA%\RenpyThiefUnofficialPatch\6.7.8Runtime`，从而保持 `6.7.8Origin` 不被写入。
...
切换 Provider 前，请先关闭游戏并正常关闭 RenpyThief，再从补丁中重新启动。当前版本不支持运行中热切换。
...
补丁把待翻译文本发到你选择的第三方平台或本机模型，启用本地 Bridge 与主进程解袋回写。界面不再提供官方额度选项。

兼容性保护开启时，会在同一套 Qt 网络钩子里本地应答会话接口并拒绝官方游戏配置下载；翻译仍只走你选择的 Bridge。
...
兼容性保护开启时，同一套钩子还会本地应答已知会话接口，并拒绝已知游戏配置/补齐下载；若工作副本还没有登录记录，启动前会写入仅用于本机的会话标记。翻译仍只走你选择的 Bridge。
...
兼容性保护开启时，补丁会本地应答已知会话接口，并在内置 6.7.8 的工作副本缺少登录记录时写入本机会话标记，不覆盖已有记录，也不改你自己的原版目录。这不是官方账号。若仍弹出登录页，请确认兼容性保护已开启。
...
本启动器只走你自己的 API，已经去掉官方额度页面。若要用原版免费翻译，请直接运行你自己的 `RenpyThief.exe`，不要和本补丁同时开。
...
在 GUI 中点击“打开诊断目录”，记录补丁版本、安装版/便携版、Windows 版本、RenpyThief 版本、Provider 和失败阶段。提交前请人工检查，避免贴出完整台词或密钥。
