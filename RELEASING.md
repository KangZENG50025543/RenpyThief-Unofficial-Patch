...
- `RenpyThiefPatch-v1.0.0-setup-x64.exe`：推荐安装版；包含开始菜单快捷方式、默认勾选的可选桌面快捷方式、使用说明入口和卸载项。
- `RenpyThiefPatch-v1.0.0-portable-x64.zip`：开箱即用便携版；完整解压后运行，不需要安装 Python。

两者必须由同一提交构建、功能一致，并对应同一个不可变标签 `v1.0.0`。不得把原版 RenpyThief、游戏文件、用户凭据或分析资料打进任何资产。v1.0.0 作为正式版发布，不要勾选 Pre-release。
...
```powershell
.\release\RenpyThiefPatch-v1.0.0-portable-x64\RenpyThiefPatch.exe --smoke-test
.\release\RenpyThiefPatch-v1.0.0-portable-x64\router\translate_bridge.exe --help
```
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\build_installer.ps1 `
  -SourceDirectory .\release\RenpyThiefPatch-v1.0.0-portable-x64 `
...
先提交 v1.0.0 修改并推送 `main`，确认工作树干净、CI 通过，再检查和标记该提交：
