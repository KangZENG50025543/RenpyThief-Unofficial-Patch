...
本版本是正式版，用来接替 v1.0.3：启动器只走用户自己的 API，界面没有官方额度页面。主进程解袋后把台词送到用户自己的 Bridge。游戏窗口和原版翻译仪表盘都能看到回写结果。当前提供两个 Windows x64 资产：
...
5. 测试覆盖安装/升级路径。启动器只走用户自己的 API，使用包内 `6.7.8Origin`，不得读写用户已有原版目录。界面没有官方额度页面。
