...
- `RenpyThiefPatch-v0.1.2-setup-x64.exe`：推荐安装版；包含开始菜单快捷方式、默认勾选的可选桌面快捷方式、使用说明入口和卸载项。
- `RenpyThiefPatch-v0.1.2-portable-x64.zip`：开箱即用便携版；完整解压后运行，不需要安装 Python。

两者必须由同一提交构建、功能一致，并对应同一个不可变标签 `v0.1.2`。不得把原版 RenpyThief、游戏文件、用户凭据或分析资料打进任何资产。

## 2. 发布前准备

- 确认版本号、README、包内 `QUICK_START.txt`、安装器显示和文件名均为 `0.1.2`。
...
release\RenpyThiefPatch-v0.1.2-setup-x64.exe
release\SHA256SUMS.txt
release\source-assets-v0.1.2\PyQt5-5.15.11.tar.gz
release\source-assets-v0.1.2\qt-everywhere-src-5.15.2.tar.xz
release\source-assets-v0.1.2\MinHook-1.3.4-c3fcafdc10146beb5919319d0683e44e3c30d537.zip
release\source-assets-v0.1.2\Python-3.12.7.tar.xz
release\source-assets-v0.1.2\SOURCE_ARCHIVES.SHA256
```

准备并复核第三方对应源码附件：

```powershell
.\scripts\prepare_release_sources.ps1 -Version 0.1.2 `
  -OutputDirectory .\release\source-assets-v0.1.2
.\scripts\prepare_release_sources.ps1 -Version 0.1.2 `
  -OutputDirectory .\release\source-assets-v0.1.2 -VerifyOnly
```

依赖版本没有变化时可以复用已验证且散列完全相同的第三方源码字节，但仍须在 v0.1.2 目录执行 `-VerifyOnly`；不得复用旧版补丁 EXE、安装器或 ZIP。
...
```powershell
.\release\RenpyThiefPatch-v0.1.2-portable-x64\RenpyThiefPatch.exe --smoke-test
.\release\RenpyThiefPatch-v0.1.2-portable-x64\router\translate_bridge.exe --help
```
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\build_installer.ps1 `
  -SourceDirectory .\release\RenpyThiefPatch-v0.1.2-portable-x64 `
...
先提交 v0.1.2 修改并推送 `main`，确认工作树干净、CI 通过，再检查和标记该提交：
...
1. 选择标签 `v0.1.2`，标题填写 `v0.1.2 — 更新保护超时改为警告并继续`。
