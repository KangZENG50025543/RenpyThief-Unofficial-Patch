...
当前 `v0.1.2` 构建基线如下；依赖版本相对 `v0.1.1` 未发生变化：
...
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\prepare_release_sources.ps1 `
  -Version 0.1.2 -OutputDirectory .\release\source-assets-v0.1.2 -DryRun

# 下载四个精确归档并逐个限制大小、校验 SHA-256
# v0.1.2 发布目录为 release\source-assets-v0.1.2，总下载量约 582 MiB，Qt 占绝大部分
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\prepare_release_sources.ps1 `
  -Version 0.1.2 -OutputDirectory .\release\source-assets-v0.1.2

# 上传前再次进行纯本地校验
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\prepare_release_sources.ps1 `
  -Version 0.1.2 -OutputDirectory .\release\source-assets-v0.1.2 -VerifyOnly
```
